...
const SECTION_PARTS = {
  s1: ['1-1','1-2'],          // 第 1 節 槓桿(一)：三要素 + 三類型
  s2: ['1-3'],                // 第 2 節 槓桿(二)：實驗 + 功不變
  s3: ['2-1','2-2'],          // 第 3 節 輪軸(一)：剖面圖 + 原理
  s4: ['2-3'],                // 第 4 節 輪軸(二)：實驗 + 應用
  s5: ['3-1'],                // 第 5 節 滑輪(一)：定/動原理
  s6: ['3-2'],                // 第 6 節 滑輪(二)：實體實驗
  s7: ['4-1','4-2','4-3'],    // 第 7 節 齒輪+綜合：傳動 + 功不變總結
};
const SECTION_LABEL = {
  s1: '第 1 節 槓桿(一)',
  s2: '第 2 節 槓桿(二)',
...
function ProgressProvider({ children }) {
  ...
  const isSectionAccessible = (section) => {
    if (section === 'textbook' || section === 's1') return true;
    const need = {
      s2: SECTION_PARTS.s1,
      s3: [...SECTION_PARTS.s1, ...SECTION_PARTS.s2],
      s4: [...SECTION_PARTS.s1, ...SECTION_PARTS.s2, ...SECTION_PARTS.s3],
      s5: [...SECTION_PARTS.s1, ...SECTION_PARTS.s2, ...SECTION_PARTS.s3, ...SECTION_PARTS.s4],
      s6: [...SECTION_PARTS.s1, ...SECTION_PARTS.s2, ...SECTION_PARTS.s3, ...SECTION_PARTS.s4, ...SECTION_PARTS.s5],
      s7: [...SECTION_PARTS.s1, ...SECTION_PARTS.s2, ...SECTION_PARTS.s3, ...SECTION_PARTS.s4, ...SECTION_PARTS.s5, ...SECTION_PARTS.s6],
      quiz: PARTS,
    }[section] || [];
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans overflow-x-hidden">
        ...
        <div className="max-w-5xl mx-auto p-4 md:p-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <nav className="flex flex-wrap gap-2 justify-center">
              ...
              <TabButton active={activeTab === 's4'} onClick={() => tryTab('s4')} label="4 輪軸(二)" locked={!isSectionAccessible('s4')} />
              <TabButton active={activeTab === 's5'} onClick={() => tryTab('s5')} label="5 滑輪(一)" locked={!isSectionAccessible('s5')} />
              <TabButton active={activeTab === 's6'} onClick={() => tryTab('s6')} label="6 滑輪(二)" locked={!isSectionAccessible('s6')} />
              <TabButton active={activeTab === 's7'} onClick={() => tryTab('s7')} label="7 齒輪+綜合" locked={!isSectionAccessible('s7')} />
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-6 my-4 bg-white rounded-xl shadow-sm border border-slate-200 min-h-[80vh]">
        ...
        {activeTab === 's4' && (isSectionAccessible('s4') ? <WheelAxleSection2 /> : <SectionLockedNotice section="s4" missing={missingFor('s4')} />)}
        {activeTab === 's5' && (isSectionAccessible('s5') ? <PulleySection1 /> : <SectionLockedNotice section="s5" missing={missingFor('s5')} />)}
        {activeTab === 's6' && (isSectionAccessible('s6') ? <PulleySection2 /> : <SectionLockedNotice section="s6" missing={missingFor('s6')} />)}
        {activeTab === 's7' && (isSectionAccessible('s7') ? <GearSection /> : <SectionLockedNotice section="s7" missing={missingFor('s7')} />)}
  ...
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <Scale className="text-blue-500" /> 第 1 節 槓桿(一)：三要素與三類型
      </h2>
...
/* =========================================
   Section s2: 第 2 節 槓桿(二)：實驗與功不變
   ========================================= */
function LeverSection2() {
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <Scale className="text-blue-500" /> 第 2 節 槓桿(二)：實驗與功不變
      </h2>
...
/* Helper: 只在 partId 通過後顯示 Wordwall */
function WordwallGameGate({ partId, src, sectionLabel }) {
  const { unlocked } = useContext(ProgressContext);
  if (!unlocked[partId]) return null;
  return <WordwallGame src={src} sectionLabel={sectionLabel} />;
}
...
/* =========================================
   Section s3: 第 3 節 輪軸(一)：剖面圖與原理
   ========================================= */
function WheelAxleSection1() {
  const { unlocked } = useContext(ProgressContext);
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <RotateCw className="text-blue-500" /> 第 3 節 輪軸(一)：剖面圖與原理
      </h2>
      ...
      {/* Wordwall：第 3 節活動（2-2 通過後出現） */}
      <WordwallGameGate partId="2-2"
        src="https://wordwall.net/tc/embed/f01f7c50374846b9a84c91f15d8ec3cc?themeId=48&templateId=5&fontStackId=0"
        sectionLabel="輪軸是輪還是軸" />
    </div>
  );
}

/* =========================================
   Section s4: 第 4 節 輪軸(二)：實驗與生活應用
   ========================================= */
function WheelAxleSection2() {
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <RotateCw className="text-blue-500" /> 第 4 節 輪軸(二)：實驗與生活應用
      </h2>
...
/* =========================================
   Section s5: 第 5 節 滑輪(一)：定/動滑輪原理
   ========================================= */
function PulleySection1() {
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <RotateCw className="text-blue-500" /> 第 5 節 滑輪(一)：定/動滑輪原理
      </h2>
...
/* =========================================
   ...
   ========================================= */
function PulleySection2() {
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <RotateCw className="text-blue-500" /> 第 6 節 滑輪(二)：實體實驗與應用
      </h2>
      ...
      <WordwallGameGate partId="3-2"
        src="https://wordwall.net/tc/embed/34a2928692324200becafd3a500078a6?themeId=23&templateId=49&fontStackId=0"
        sectionLabel="滑輪" />
    </div>
  );
}
...
function GearSection() {
  ...
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <Settings className="text-blue-500" /> 第 7 節 齒輪+綜合：傳動與功不變總結
      </h2>
    ...
    <div className="mt-10 bg-gradient-to-br from-blue-50 via-white to-purple-50 rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="p-6">
        <p className="mb-6 text-slate-700 text-base leading-relaxed">
          七節課我們認識了 <strong>槓桿、輪軸、滑輪、齒輪與鏈條</strong> 四大簡單機械。把它們放在一起比較,就能看出每一種機械的特長與限制!
        </p>
        <div className="overflow-x-auto">
