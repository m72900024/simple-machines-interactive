...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans overflow-x-hidden">
      {/* Header */}
      <header className="bg-blue-600 text-white p-6 shadow-md sticky top-0 z-50">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center">
          <div>
            ...
            <p className="mt-2 text-blue-100 text-sm md:text-base">第一單元互動學習網 ｜ 電子課本 × 動態圖解 × 虛擬實驗室</p>
          </div>
          <nav className="mt-4 md:mt-0 flex flex-wrap gap-2 justify-center">
            <TabButton active={activeTab === 'textbook'} onClick={() => setActiveTab('textbook')} label="📚 電子課本" />
            <TabButton active={activeTab === 'lever'} onClick={() => setActiveTab('lever')} label="1. 槓桿" />
            <TabButton active={activeTab === 'wheel'} onClick={() => setActiveTab('wheel')} label="2. 輪軸與滑輪" />
            <TabButton active={activeTab === 'gear'} onClick={() => setActiveTab('gear')} label="3. 齒輪與鏈條" />
            <TabButton active={activeTab === 'quiz'} onClick={() => setActiveTab('quiz')} label="單元總測驗" isQuiz />
          </nav>
        </div>
      </header>
...
function TabButton({ active, onClick, label, isQuiz }) {
  return (
    <button
      onClick={onClick}
      className={`px-4 py-2 rounded-lg font-medium transition-colors ${
        active
          ? 'bg-white text-blue-600 shadow-sm border-b-4 border-blue-400'
          : isQuiz
            ? 'bg-orange-500 text-white hover:bg-orange-400 border-b-4 border-orange-700'
            : 'bg-blue-700 text-white hover:bg-blue-500 border-b-4 border-blue-800'
      } hover:translate-y-px active:translate-y-1 active:border-b-0 mb-1`}
    >
      {label}
    </button>
  );
...
function LeverSection() {
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <Scale className="text-blue-500" /> 活動 1:如何應用槓桿原理
      </h2>
...
function WheelSection() {
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <RotateCw className="text-blue-500" /> 活動 2:輪軸與滑輪如何便利生活
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <Card title="重點整理:滑輪兄弟" icon={<BookOpen className="text-green-500" />}>
          ...
          <ul className="space-y-2 list-disc ml-5 text-slate-700">
            <li><strong>定滑輪:</strong>固定在一個地方不移動。<br/><span className="text-blue-600 font-bold">不省力</span>,但能<span className="text-blue-600 font-bold">改變施力方向</span>。</li>
            <li><strong>動滑輪:</strong>會跟著重物一起上下移動。<br/>能<span className="text-green-600 font-bold">省一半的力</span>,但<span className="text-red-500 font-bold">不能改變施力方向</span>。</li>
          </ul>
        </Card>
      </div>
...
function GearSection() {
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold text-blue-800 mb-6 flex items-center gap-2 border-b pb-2">
        <Settings className="text-blue-500" /> 活動 3:齒輪與鏈條傳送動力
      </h2>
